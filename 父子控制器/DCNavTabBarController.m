...
-(void)addSliderView {
    ...
    UIView *slider = [[UIView alloc]initWithFrame:CGRectMake(0,41-64,self.btnW, 5)];
...
-(void)addVCView
{
    ...
    self.contentView.bounces = NO;//不会反弹,YES时滚动到边缘可超为超出界面外一点然后弹回原位,NO不会超出可视范围
...
-(void)click:(UIButton *)sender {
    ...
    //sender.tag != self.VCArr.count + 10000 - 1判断当前点击的btn按钮是不是最后一个按钮
...
//判断是否切换导航条按钮的状态(字体放大的效果)
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    CGFloat offX =  scrollView.contentOffset.x;
//        int tag = (int)(offX /DCScreenW + .5) + 10000;
    int tag = (int)(offX /DCScreenW ) + 10000;
